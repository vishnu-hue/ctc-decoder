...
class CMakeBuild(build_ext):
    def run(self):
        ...
        if version.parse(cmake_version) < version.parse("3.5.1"):
            raise RuntimeError("CMake >= 3.5.1 is required to build ctcdecoder")

        # our CMakeLists builds all the extensions at once
        self.build_extensions()

    def build_extensions(self):
        extdir = os.path.abspath("ctcdecoder")
        ...
        if platform.system() == "Windows":
            ...
            #     cmake_args += ["-A", "x64"]
            # build_args += ["--", "/m"]
            raise RuntimeError("ctcdecoder doesn't support building on Windows yet")
        else:
...
setup(
    ...
    version="0.0.1",
    author="vishnu-hue",
    description="ctcdecoder bindings for python",
    long_description="",
    ext_modules=[
        CMakeExtension("ctcdecoder._common"),
        CMakeExtension("ctcdecoder._decoder"),
        CMakeExtension("ctcdecoder._decoderfree"),
    ],
